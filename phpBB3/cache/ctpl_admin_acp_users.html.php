...
<?php if ($this->_rootref['S_SELECT_USER']) {  ?>
	...
	<dl>
		<dt><label for="username"><?php echo ((isset($this->_rootref['L_ENTER_USERNAME'])) ? $this->_rootref['L_ENTER_USERNAME'] : ((isset($user->lang['ENTER_USERNAME'])) ? $user->lang['ENTER_USERNAME'] : '{ ENTER_USERNAME }')); ?>:</label></dt>
...
<?php } if ($this->_rootref['S_OVERVIEW']) {  $this->_tpl_include('acp_users_overview.html'); } else if ($this->_rootref['S_FEEDBACK']) {  $this->_tpl_include('acp_users_feedback.html'); } else if ($this->_rootref['S_WARNINGS']) {  $this->_tpl_include('acp_users_warnings.html'); } else if ($this->_rootref['S_PROFILE']) {  $this->_tpl_include('acp_users_profile.html'); } else if ($this->_rootref['S_PREFS']) {  $this->_tpl_include('acp_users_prefs.html'); } else if ($this->_rootref['S_AVATAR']) {  $this->_tpl_include('acp_users_avatar.html'); } else if ($this->_rootref['S_RANK']) {  ?>
...
<?php } else if ($this->_rootref['S_SIGNATURE']) {  $this->_tpl_include('acp_users_signature.html'); } else if ($this->_rootref['S_GROUPS']) {  ?>
	...
	<?php $_group_count = (isset($this->_tpldata['group'])) ? sizeof($this->_tpldata['group']) : 0;if ($_group_count) {for ($_group_i = 0; $_group_i < $_group_count; ++$_group_i){$_group_val = &$this->_tpldata['group'][$_group_i]; if ($_group_val['S_NEW_GROUP_TYPE']) {  ?>
		...
		<?php } else { if (!($_group_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
				...
				<td><?php if ($_group_val['S_IS_MEMBER']) {  if ($_group_val['S_NO_DEFAULT']) {  ?><a href="<?php echo $_group_val['U_DEFAULT']; ?>"><?php echo ((isset($this->_rootref['L_GROUP_DEFAULT'])) ? $this->_rootref['L_GROUP_DEFAULT'] : ((isset($user->lang['GROUP_DEFAULT'])) ? $user->lang['GROUP_DEFAULT'] : '{ GROUP_DEFAULT }')); ?></a><?php } else { ?><strong><?php echo ((isset($this->_rootref['L_GROUP_DEFAULT'])) ? $this->_rootref['L_GROUP_DEFAULT'] : ((isset($user->lang['GROUP_DEFAULT'])) ? $user->lang['GROUP_DEFAULT'] : '{ GROUP_DEFAULT }')); ?></strong><?php } } else if (! $_group_val['S_IS_MEMBER'] && $_group_val['U_APPROVE']) {  ?><a href="<?php echo $_group_val['U_APPROVE']; ?>"><?php echo ((isset($this->_rootref['L_GROUP_APPROVE'])) ? $this->_rootref['L_GROUP_APPROVE'] : ((isset($user->lang['GROUP_APPROVE'])) ? $user->lang['GROUP_APPROVE'] : '{ GROUP_APPROVE }')); ?></a><?php } else { ?>&nbsp;<?php } ?></td>
				<td><?php if ($_group_val['S_IS_MEMBER'] && ! $_group_val['S_SPECIAL_GROUP']) {  ?><a href="<?php echo $_group_val['U_DEMOTE_PROMOTE']; ?>"><?php echo $_group_val['L_DEMOTE_PROMOTE']; ?></a><?php } else { ?>&nbsp;<?php } ?></td>
