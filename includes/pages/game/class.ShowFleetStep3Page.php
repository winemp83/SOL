 ...
 * @info $Id: class.ShowFleetStep3Page.php 2604 2013-01-18 11:35:42Z slaver7 $
 * @link http://2moons.cc/
 */
...
require_once(ROOT_PATH . 'includes/functions/EventAttack.php');
...
class ShowFleetStep3Page extends AbstractPage
{
	...
	public function show()
	{
		...
		$targetPlanetData  	= $GLOBALS['DATABASE']->getFirstRow("SELECT id, id_owner, der_metal, der_crystal, destruyed, ally_deposit, ev_attack FROM ".PLANETS." WHERE universe = ".$UNI." AND galaxy = ".$targetGalaxy." AND system = ".$targetSystem." AND planet = ".$targetPlanet." AND planet_type = '".($targetType == 2 ? 1 : $targetType)."';");
...
// Spezial-Event-Mod 1.7.2 check EventAttack from SpezialEvent	
     
	 if($targetMission == 1)
	{

		if($targetPlanetData['ev_attack']==1) 
		{		
			EventAttack($targetGalaxy, $targetSystem, $targetPlanet, $targetType, $targetMission, $fleetRessource, $fleetGroup, $fleetSpeed, $distance);
		}
	}
