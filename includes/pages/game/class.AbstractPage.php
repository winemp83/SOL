...
abstract class AbstractPage 
{
	...
	protected function getNavigationData() 
    {
		...
		$Marketlot = $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".MARKET." WHERE universe = 1;");
		if (!empty($Marketlot)) {
		$this->tplObj->assign_vars(array(
		'ddd' => $Marketlot,
		    ));
		...
		$Alarm       = $GLOBALS['DATABASE']->uniquequery("SELECT `fleet_mission` FROM ".FLEETS." WHERE `fleet_target_owner` = '". $USER['id'] ."' AND (`fleet_mission` = '1' OR `fleet_mission` = '2' OR `fleet_mission` = '6') AND `fleet_mess` ='0';");
            ...
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/images/red.gif" >',
                        ));
            }
            if($Alarm['fleet_mission'] != 1 && $Alarm['fleet_mission'] != 2 && $Alarm['fleet_mission'] != 6){
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/images/green.gif" >',
                        ));
                }
            ...
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/images/blue.gif" >',
                        ));
            }
         if($USER['race'] == 1){
        	$USER['race'] =    $LNG['Race1'];}
