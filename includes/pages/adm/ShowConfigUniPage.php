...
function ShowConfigUniPage()
{
	...
	if (!empty($_POST))
	{
		...
		$tfev_aktiv				= isset($_POST['tfev_aktiv']) && $_POST['tfev_aktiv'] == 'on' ? 1 : 0;
        $tfev_info		 		= HTTP::_GP('tfev_info', '', true);	
...
// Spezial-Event-Mod 1.7.2
	...
	$template->assign_vars(array(
		...
		'alliance_create_min_points' 	=> $CONF['alliance_create_min_points'],
		// Spezial-Event-Mod 1.7.2			
	    'tfev_aktiv'					=> $CONF['tfev_aktiv'],
		'ev_eventplaneten'				=> $LNG['ev_eventplaneten'],
		'tfev_info'						=> $CONF['tfev_info'],
		'tf_info_ev'					=> $LNG['tf_info_ev']			
	));
