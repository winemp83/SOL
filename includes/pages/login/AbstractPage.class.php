...
abstract class AbstractPage 
{
    ...
    {		
		...
		#Block für online user, gesamt user, letzter user		
		$sql = "SELECT * FROM uni1_users";
		$result = $GLOBALS['DATABASE']->query($sql);
		$j = 0;
		$i = 0;
		$help2 = 0;
		foreach ($result as $online ) {
			$help1 = $online['register_time'];
			if($help2 < $help1){
				$help2 = $help1;
			}
			$helptime = time() - 300;
			if($online['onlinetime'] > $helptime){
				$i++;
			}
			$j++;
		}
		$sql1 = "SELECT `username` FROM uni1_users WHERE register_time='".$help2."'";
		$result1 = $GLOBALS['DATABASE']->query($sql1);		
		foreach ($result1 as $lastuser){
			$last_user = $lastuser['username'];
		}
		
		$news = '';
		#Block für die news anzeige:
		$sql = "SELECT * FROM ".NEWS." WHERE id != '0' ORDER BY 'id' ASC";
		$result = $GLOBALS['DATABASE']->query($sql);
		foreach($result as $NEWS){
			if($NEWS['start_page'] != 0){
				$news = $NEWS['text'];
			}
		}
		$help = trim(substr($news,0,80));
		
		$news = '<div align="center" width="center">'.$help .'...<a href="index.php?page=SingleNews" target="_self"> <span style="color:gold;">(mehr)</span> </a></div>';
